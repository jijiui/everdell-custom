...
export class GameState {
  ...
  // PlayerId who owns the current undo snapshot (if any).
  private undoOwnerPlayerId: string | null;
  ...
  constructor({
    ...
    this.undoOwnerPlayerId = undoOwnerPlayerId;
  ...
  getUndoOwnerPlayerId(): string | null {
    return this.undoOwnerPlayerId ?? null;
  }
  ...
  toJSON({
    ...
    return cloneDeep({
      ...{
        ...
        undoOwnerPlayerId: this.undoOwnerPlayerId ?? null,
  ...
  next(gameInput: GameInput, autoAdvance = true, skipUndo = false): GameState {
    // Invalidate cross-turn undo when next active player acts.
    if (gameInput.inputType !== GameInputType.UNDO) {
      if (
        this.gameStateForUndo &&
        this.undoOwnerPlayerId &&
        this.undoOwnerPlayerId !== this.activePlayerId
      ) {
        this.gameStateForUndo = null;
        this.undoOwnerPlayerId = null;
      }
    }
    if (gameInput.inputType === GameInputType.UNDO) {
      ...
      nextState.undoOwnerPlayerId = null;
      return nextState;
    }
  ...
  private nextInner(
    ...
    if (this.pendingGameInputs.length === 0) {
      // Do not clear undo snapshot here to allow previous player
      // a one-time cross-turn undo before next player acts.
      this.nextPlayer();
    }
  ...
  static fromJSON(gameStateJSON: GameStateJSON): GameState {
    return new GameState({
      ...
      gameStateForUndo: gameStateJSON.gameStateJSONForUndo
        ...
        : null,
      undoOwnerPlayerId: gameStateJSON.undoOwnerPlayerId ?? null,
    });
  }
  ...
  getPossibleGameInputs(): GameInput[] {
    ...
    if (this.gameStateForUndo && this.undoOwnerPlayerId === this.activePlayerId) {
